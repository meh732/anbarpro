...
export const DashboardView: React.FC = () => {
  ...
  const tiles = [
    ...
    { id: 'kardex', label: 'کاردکس قطعات و کالاها', icon: ClipboardList, color: 'from-indigo-500 to-purple-700', shadow: 'shadow-indigo-500/30' },
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 glass-card rounded-[2rem] p-6 relative overflow-hidden group">
          ...
          <div className="flex items-center justify-between mb-6 relative z-10">
            ...
            <div className="flex items-center gap-4 bg-white/40 border border-white/50 p-1.5 rounded-xl backdrop-blur-md">
              <span className="px-3 py-1 bg-white/75 shadow-xs border border-white/80 rounded-lg text-sm font-bold text-indigo-600">هفته جاری</span>
          ...
          <div className="h-64 w-full relative z-10" dir="ltr">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={chartData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                ...
                <Tooltip 
                  contentStyle={{ borderRadius: '1rem', border: '1px solid rgba(255,255,255,0.6)', background: 'rgba(255,255,255,0.85)', backdropFilter: 'blur(10px)', boxShadow: '0 10px 25px -5px rgba(0, 0, 0, 0.05)', fontWeight: 'bold' }}
        ...
        <div className="flex flex-col gap-6">
          <div className="bg-gradient-to-br from-rose-500/95 to-pink-600/95 border border-white/20 backdrop-blur-md rounded-[2rem] p-6 text-white shadow-lg shadow-rose-500/10 relative overflow-hidden group hover:scale-[1.01] transition-all ios-press duration-300">
          ...
          <div className="glass-card rounded-[2rem] p-6 flex-1 flex flex-col relative overflow-hidden group">
            <div className="absolute -bottom-10 -left-10 w-40 h-40 bg-amber-500/5 rounded-full blur-3xl opacity-60"></div>
            ...
            <div className="mt-auto relative z-10">
              <button onClick={() => setActiveTab('requests')} className="w-full py-3 rounded-xl bg-white/40 hover:bg-white/70 text-slate-700 font-bold text-sm transition-all border border-white/50 flex items-center justify-center gap-2 ios-press">
                بررسی درخواست‌ها
        ...
        {tiles.map((tile) => {
          ...
          return (
            <button
              ...
              className="relative overflow-hidden group rounded-[2rem] p-5 md:p-6 text-right flex flex-col items-start justify-between min-h-[165px] cursor-pointer glass-card ios-press shadow-xs"
            >
              {/* Glassmorphism abstract shape */}
              <div className="absolute top-0 right-0 -mr-8 -mt-8 w-32 h-32 bg-gradient-to-br from-indigo-500/5 to-purple-500/5 opacity-50 rounded-full blur-2xl group-hover:opacity-100 transition-opacity"></div>
              
              <div className={`bg-gradient-to-br ${tile.color} p-3.5 rounded-2xl mb-4 group-hover:scale-110 transition-transform shadow-md shadow-slate-200 border border-white/20`}>
                <Icon className="w-6 h-6 text-white" />
              </div>
              
              <div>
                <h3 className="text-slate-800 font-black text-base tracking-tight leading-tight group-hover:text-indigo-600 transition-colors">{tile.label}</h3>
                <p className="text-[11px] text-indigo-950/70 mt-1 font-extrabold tracking-wide">بخش عملیاتی</p>
              </div>
              ...
              <div className="absolute bottom-5 left-5 opacity-0 group-hover:opacity-100 transition-all transform translate-x-4 group-hover:translate-x-0">
                <div className="w-7 h-7 rounded-full bg-indigo-50 text-indigo-600 flex items-center justify-center border border-indigo-100 shadow-sm">
